 ...
 * configJson is stored as an opaque JSON string — it is parsed and exposed as
...
function AuditEntryRow({ entry }: { entry: AuditLogEntry }) {
  ...
  return (
    <div className="px-3 py-2 space-y-1">
      ...
      <div className="text-slate-500 flex gap-4 flex-wrap">
        <span>{entry.entityType}{entry.entityId ? ` · ${entry.entityId.slice(0, 8)}…` : ''}</span>
        {entry.ipAddress && <span>from {entry.ipAddress}</span>}
        {summaryBits.length > 0 && <span className="font-medium text-slate-600">{summaryBits.join(', ')}</span>}
      </div>

      {(before || after) && changes.length === 0 && (
        <p className="text-slate-400 italic">No field differences captured.</p>
      )}
      {changes.length > 0 && (
        <details className="mt-1">
          <summary className="cursor-pointer text-indigo-600 hover:underline">View changes</summary>
          <ul className="mt-1 space-y-1 p-2 bg-slate-50 rounded text-[11px]">
            {changes.map(change => (
              <li key={change.key} className="break-words">
                {change.kind === 'added' && (
                  <span>
                    <span className="text-green-700 font-medium">+ {change.key}</span>
                    <span className="text-slate-600"> = {formatValue(change.after)}</span>
                  </span>
                )}
                {change.kind === 'removed' && (
                  <span>
                    <span className="text-red-600 font-medium">− {change.key}</span>
                    <span className="text-slate-600"> = {formatValue(change.before)}</span>
                  </span>
                )}
                {change.kind === 'changed' && (
                  <span className="block">
                    <span className="text-slate-700 font-medium">{change.key}</span>:{' '}
